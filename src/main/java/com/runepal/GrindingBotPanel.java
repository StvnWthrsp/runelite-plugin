...
public class GrindingBotPanel extends PluginPanel {
    ...
    public GrindingBotPanel(RunepalPlugin plugin, BotConfig config, ConfigManager configManager) {
        super();
        this.configManager = configManager;
        this.config = config;

        this.itemIdSpinner = new JSpinner(new SpinnerNumberModel(config.grindingItemId(), 0, Integer.MAX_VALUE, 1));
...
//        configPanel.setBorder(BorderFactory.createTitledBorder("Basic Sand Crab Configuration"));
        ...
        itemIdSpinner.setToolTipText("ID of Item to grind");
        configPanel.add(itemIdSpinner, gbc);

        // Add event listeners
        itemIdSpinner.addChangeListener(e -> saveConfiguration());

        return configPanel;
    }

    private void saveConfiguration() {
        configManager.setConfiguration("runepal", "grindingItemId", (Integer) itemIdSpinner.getValue());
    }

    private void loadConfigurationValues() {
        itemIdSpinner.setValue(config.grindingItemId());
    }
